...
pn = 'C:\rag-net-v2-master\testingDataset\test_images\';
...
for ii = 1:nfiles
    ...
    i2 = imresize(i2,[480 1280],'bilinear');
...
%     img = mat2gray(mask.*double(oimg));
    ...
    i2(:,:,1) = (uint8(mask).*(i2(:,:,1)));
    i2(:,:,2) = (uint8(mask).*(i2(:,:,2)));
    i2(:,:,3) = (uint8(mask).*(i2(:,:,3)));
    
    i2 = imresize(i2,[576 768],'bilinear');
%     imshow(i2)
    % create folder with name "Preprocessed" in the preprocessing directory
    % before saving the preprocessed image 
    imwrite(i2,[pn 'processed\' fn],'PNG');
end
